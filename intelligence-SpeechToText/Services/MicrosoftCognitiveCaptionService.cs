namespace ImageCaption.Services
{
    ...
    public class MicrosoftCognitiveCaptionService : ICaptionService
    {
        ...
        /// <returns></returns>
        public async Task<string> GetOcrTextAsync(Stream stream)
        {
            var client = new VisionServiceClient(ApiKey);
            var result = await client.AnalyzeImageAsync(stream, VisualFeatures);
            return ProcessAnalysisResult2(result);
        ...
        private static string ProcessAnalysisResult(AnalysisResult result)
       {
        ...
        /// <returns></returns>
        private static string ProcessAnalysisResult2(AnalysisResult result)
        {
            string message = result?.Adult?.IsAdultContent + result?.Description?.Captions.FirstOrDefault()?.Text;
            
            //.Description?.Captions.FirstOrDefault()?.Text;

            return string.IsNullOrEmpty(message) ?
                        "Couldn't find a person for this one" :
                        "I think it's " + message;
        }
    }
}